...
function addReportFooter(doc, documentId, generatedAtLabel = null) {
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    ...
    const footerText = generatedAtLabel
      ? `Generated by Shooting Records | Generated: ${generatedAtLabel} | Doc ID: ${documentId} | Page ${page} of ${totalPages}`
      : `Generated by Shooting Records | Doc ID: ${documentId} | Page ${page} of ${totalPages}`;
...
function drawRecordHeader(doc, title, cursor) {
  cursor = ensureSpace(doc, cursor, 18);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(10.5);
  doc.setTextColor(...REPORT.navy);
  doc.text(title, REPORT.margin + 2, cursor.y);
  return { ...cursor, y: cursor.y + 5 };
}

function drawTargetOverviewRecord(doc, record, reportData, cursor) {
  const { rifles = {}, clubs = {}, locations = {} } = reportData.lookups;
  cursor = ensureSpace(doc, cursor, 82);
  cursor = drawRecordHeader(doc, `Target Shooting — ${formatDate(record.date)}`, cursor);
  ...
  cursor = drawClayCompactCard(doc, multiFirearmList ? 'Firearms' : 'Firearm Used', multiFirearmList ? [
    { label: 'Firearms', value: multiFirearmList },
  ] : [
    { label: 'Firearm', value: formatSingleTargetValue(record, rifles, 'name') },
    { label: 'Make', value: formatSingleTargetValue(record, rifles, 'make') },
    { label: 'Model', value: formatSingleTargetValue(record, rifles, 'model') },
    { label: 'Calibre', value: formatSingleTargetValue(record, rifles, 'caliber') },
    { label: 'Serial Number', value: formatSingleTargetValue(record, rifles, 'serial') },
    { label: 'Rounds Fired', value: formatSingleTargetValue(record, rifles, 'rounds') },
    { label: 'Ammunition', value: formatSingleTargetValue(record, rifles, 'ammo') },
  ], cursor);
  return { ...cursor, y: cursor.y + 3 };
}
...
function drawClayOverviewRecord(doc, record, reportData, cursor) {
  ...
  const shotgun = shotguns[record.shotgun_id] || {};
  cursor = ensureSpace(doc, cursor, 82);
  cursor = drawRecordHeader(doc, `Clay Shooting — ${formatDate(record.date)}`, cursor);
  cursor = drawClayCompactCard(doc, 'Activity Details', [
    { label: 'Date', value: formatDate(record.date) },
    { label: 'Club / Range', value: resolveActivityLocation(record, clubs, locations) },
    { label: 'Check-In', value: record.checkin_time || record.start_time },
  ...
  cursor = drawClayCompactCard(doc, 'Shotgun Used', [
    { label: 'Shotgun', value: joinClean([shotgun.name, shotgun.make, shotgun.model], ' ') },
    ...
    { label: 'Cartridges Fired', value: record.rounds_fired || 0 },
    { label: 'Ammunition / Cartridge', value: joinClean([record.ammunition_used, record.shot_size, record.load_grams ? `${record.load_grams}g` : null], ' ') },
  ], cursor);
  return { ...cursor, y: cursor.y + 3 };
}
...
function drawDeerOverviewRecord(doc, record, reportData, cursor) {
  ...
  cursor = drawRecordHeader(doc, `Deer Management — ${formatDate(record.date)}`, cursor);
  cursor = drawClayCompactCard(doc, 'Activity Details', [
    { label: 'Date', value: formatDate(record.date) },
    { label: 'Land / Location', value: record.location_name || record.place_name || locations[record.location_id]?.name || locations[record.location_id]?.place_name },
    { label: 'Check-In', value: record.start_time || record.checkin_time },
    { label: 'Check-Out', value: record.end_time || record.checkout_time },
    { label: 'Notes', value: formatActivityNotes(record) },
  ], cursor);
  cursor = drawClayCompactCard(doc, 'Rifle / Cull Details', [
    ...
    { label: 'Ammunition', value: record.ammunition_used },
    { label: 'Rounds Fired', value: record.rounds_fired || record.total_count || record.number_shot || 0 },
    { label: 'Deer Species', value: record.deer_species || (record.species_list?.length ? record.species_list.map((item) => item.species).join(', ') : null) },
