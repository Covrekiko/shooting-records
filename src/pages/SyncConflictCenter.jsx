    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-3 pt-2 md:pt-4 pb-8 mobile-page-padding">
        <div className="flex items-start justify-between gap-3 mb-5">
          ...
          <button onClick={handleSync} disabled={!!action} className="px-3 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-semibold flex items-center gap-2 disabled:opacity-60">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          {[
            ...
            ['Expired', counts[SYNC_STATUS.EXPIRED] || 0],
          ].map(([label, value]) => (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${STATUS_STYLES[entry.status] || STATUS_STYLES[SYNC_STATUS.PENDING]}`}>{statusLabel(entry.status)}</span>
                        <span className="text-xs text-muted-foreground">{entry.entityName || 'Unknown module'} · {entry.action || 'operation'}</span>
                      </div>
