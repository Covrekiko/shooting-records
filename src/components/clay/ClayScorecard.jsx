...
import { X, Plus, Target, ChevronDown, ChevronUp, Trash2, Pencil, Download } from 'lucide-react';
...
function StandForm({ stand, standNumber, onSave, onCancel }) {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
      <h3 className="font-bold text-base">Stand {form.stand_number}</h3>
    ...
    <div className="bg-card border border-border rounded-2xl p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="flex-1 bg-secondary rounded-xl p-3 text-center">
          <p className="text-2xl font-black text-primary">{stand.hits}</p>
          <p className="text-xs text-muted-foreground">Hits</p>
        </div>
        <div className="text-muted-foreground font-bold">/</div>
        <div className="flex-1 bg-secondary rounded-xl p-3 text-center">
          <p className="text-2xl font-black">{stand.clays_total}</p>
          <p className="text-xs text-muted-foreground">Clays</p>
        </div>
        <div className="flex-1 bg-secondary rounded-xl p-3 text-center">
      ...
      <div className="grid grid-cols-3 gap-2">
        <motion.button whileTap={{ scale: 0.92 }} onClick={onHit}
          className="py-3.5 bg-emerald-500 text-white rounded-xl font-bold text-sm active:bg-emerald-600 transition-colors">
          + Hit
        </motion.button>
        <motion.button whileTap={{ scale: 0.92 }} onClick={onMiss}
          className="py-3.5 bg-red-500 text-white rounded-xl font-bold text-sm active:bg-red-600 transition-colors">
          + Miss
        </motion.button>
      ...
      {stand.shots_used > stand.clays_total && (
        <p className="text-xs text-muted-foreground mt-2">Shots used: {stand.shots_used} (includes re-shoots/pairs)</p>
      )}
...
export default function ClayScorecard({ session, shotguns, ammunition, onClose }) {
  ...
  // For undo: track last action per stand
  ...
  const loadScorecard = async () => {
    ...
    let sc = existing[0];
    if (!sc) {
      sc = await base44.entities.ClayScorecard.create({ clay_session_id: session.id });
    }
    setScorecard(sc);
  ...
  const saveScorecard = async (newStands) => {
    const stats = calcStats(newStands);
    await base44.entities.ClayScorecard.update(scorecard.id, {
      ...
      <div className="flex-1 bg-background overflow-y-auto">
        ...
        <div className="sticky top-0 z-10 bg-background border-b border-border px-4 py-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{session.location_name} · {session.date} · {session.checkin_time || session.start_time}</p>
          </div>
        ...
        <div className="px-4 py-4 pb-8 space-y-4 max-w-xl mx-auto">
          ...
          <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4">
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-xs">
              ...
              <div><p className="font-bold text-base">{stats.totalMisses}</p><p className="text-muted-foreground">Misses</p></div>
          ...
          {/* Add Stand */}
          {showAddForm ? (
            <StandForm standNumber={stands.length + 1} onSave={handleAddStand} onCancel={() => setShowAddForm(false)} />
          ) : (
