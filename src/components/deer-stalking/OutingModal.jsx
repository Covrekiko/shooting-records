...
export default function OutingModal({ onClose, onSubmit, selectedArea = null }) {
  ...
  const inputCls = 'w-full h-14 pl-14 pr-4 rounded-xl border border-[#F1D2A8] bg-white text-slate-900 text-sm font-medium outline-none focus:border-[#B85C00] focus:ring-2 focus:ring-[#B85C00]/10 transition-all';
  const labelCls = 'text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2 block';
  const fieldIconCls = 'absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-xl bg-[#FFF3E4] text-[#A95402] flex items-center justify-center pointer-events-none';
  ...
  return (
    <GlobalModal
      ...
      title={(
        <span className="flex items-center gap-4 min-w-0">
          <span className="w-12 h-12 rounded-full bg-[#B85C00] text-white flex items-center justify-center text-2xl shadow-sm flex-shrink-0">🦌</span>
          <span className="min-w-0 block">
            <span className="text-2xl font-bold leading-tight text-slate-900 block">Start Outing</span>
            <span className="text-sm font-medium text-[#A95402]/80 block mt-1">Stalk Map check-in</span>
          </span>
        </span>
      )}
      ...
      footer={(
        <>
          ...
          <button type="submit" className="flex-1 h-12 rounded-xl font-bold text-sm bg-[#B85C00] text-white hover:bg-[#9A4A00] transition-colors active:scale-95 flex items-center justify-center gap-3 shadow-sm">
    ...
    >
      <div className="space-y-4">
        <section className="rounded-2xl border border-[#F1D2A8] bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            <Calendar className="w-4 h-4 text-[#A95402]" />
            <h3 className="text-sm font-bold text-[#A95402] uppercase tracking-wide">Session Setup</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              {selectedAreaRecord && (
                <p className="text-xs text-[#A95402] mt-2 flex items-center gap-1.5 font-medium">
        ...
        <section className={`relative overflow-hidden rounded-2xl border p-5 ${selectedAreaRecord ? 'border-[#F1D2A8] bg-[#FFF3E4]' : 'border-slate-200 bg-slate-50'}`}>
          <div className="flex items-center gap-4 relative z-10">
            <div className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 ${selectedAreaRecord ? 'bg-[#B85C00] text-white' : 'bg-slate-200 text-slate-500'}`}>
              <Check className="w-6 h-6" />
            </div>
            <div className="min-w-0">
              <p className={`text-base font-bold ${selectedAreaRecord ? 'text-[#A95402]' : 'text-slate-700'}`}>{selectedAreaRecord ? 'Ready to check in' : 'Select an area to continue'}</p>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-700 mt-2">
                {selectedAreaRecord && <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-[#A95402]" /> Area: <strong>{selectedAreaRecord.name}</strong></span>}
                <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-[#A95402]" /> Time: <strong>{data.start_time}</strong></span>
              </div>
            </div>
          </div>
