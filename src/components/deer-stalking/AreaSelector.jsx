...
export default function AreaSelector({ savedAreas, selectedAreaId, onSelectArea }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <MapPin className="w-3 h-3 sm:w-4 sm:h-4 text-amber-300 flex-shrink-0" />
        <span className="font-medium text-xs sm:text-sm flex-1 text-left truncate text-white">
          {selectedArea?.name || 'Select Area'}
        </span>
        <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 text-white/60 transition-transform flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white/15 border border-white/20 rounded-lg shadow-lg z-[9997] max-h-48 overflow-y-auto" style={{ WebkitBackdropFilter: 'blur(20px)', backdropFilter: 'blur(20px)' }}>
          {savedAreas.length === 0 ? (
            <div className="px-4 py-3 text-sm text-white/60">No saved areas</div>
          ) : (
            savedAreas.map((area) => (
              <button
                key={area.id}
                ...
                className={`w-full text-left px-4 py-2 hover:bg-white/20 transition-colors border-b border-white/10 last:border-b-0 ${
                  selectedAreaId === area.id ? 'bg-white/25 text-white font-medium' : 'text-white/80'
                }`}
              >
